...
def draw_vehicle(surface, x, y, scale, body_color):
    ...
    pygame.draw.rect(surface, body_color, body_rect,
                     border_radius=max(3, int(8 * DIAGRAM_SCALE)))
    ...
    pygame.draw.rect(surface, VEHICLE_WINDOW, cabin_rect,
                     border_radius=max(2, int(6 * DIAGRAM_SCALE)))
    pygame.draw.rect(surface, body_color, cabin_rect,
                     width=1, border_radius=max(2, int(6 * DIAGRAM_SCALE)))
...
def draw_input_main_line(surface, label_font, show_labels=True):
    ...
    label_rect = pygame.Rect(first_rect.x, first_rect.y - MAIN_LABEL_OFFSET_Y,
                             last_rect.right - first_rect.x, LABEL_BLOCK_HEIGHT)
    pygame.draw.rect(surface, ACCENT_GREEN, label_rect, border_radius=4)
    draw_text(surface, "INPUT MAIN", label_font, (255, 255, 255),
              label_rect.centerx, label_rect.centery, center=True)

    rail_y    = first_rect.y + PITCH_HEIGHT / 2 - 5
    rail_rect = pygame.Rect(first_rect.x, int(rail_y),
...
def draw_branch_line(surface, label_font, branch_name, label, prefix,
    ...
    label_rect = pygame.Rect(rect1.x, rect1.y - BRANCH_LABEL_OFFSET_Y,
                             rect_last.right - rect1.x, LABEL_BLOCK_HEIGHT)
    pygame.draw.rect(surface, ACCENT_GREEN, label_rect, border_radius=4)
    draw_text(surface, label, label_font, (255, 255, 255),
              label_rect.centerx, label_rect.centery, center=True)

    rail_y    = rect1.y + PITCH_HEIGHT / 2 - 5
    rail_rect = pygame.Rect(rect1.x, int(rail_y),
                            rect_last.right - rect1.x, 10)
    pygame.draw.rect(surface, RAIL_COLOR, rail_rect, border_radius=3)

    for i in range(1, last_pitch + 1):
        draw_pitch(surface, label_font, i, branch_pitch_rect(branch_name, i),
                   prefix, show_labels=show_labels)
...
def draw_output_main_line(surface, label_font, show_labels=True):
    ...
    label_rect = pygame.Rect(first_rect.x, first_rect.y - OUT_LABEL_OFFSET_Y,
                             last_rect.right - first_rect.x, LABEL_BLOCK_HEIGHT)
    pygame.draw.rect(surface, ACCENT_GREEN, label_rect, border_radius=4)
    draw_text(surface, "OUTPUT MAIN", label_font, (255, 255, 255),
              label_rect.centerx, label_rect.centery, center=True)
...
#                                            |  [Present] [Auto Demo]
# ---------------------------------------------------------------
def draw_header_controls(surface, fonts, buttons,
                         blue_speed_factor, yellow_speed_factor,
...
def draw_legend(surface, fonts, paused, blue_speed_factor, yellow_speed_factor,
                fullscreen, presentation_mode, auto_demo_mode):
    ...
    box_w = padding * 2 + icon_offset + max_left_width + column_spacing + max_right_width
    box_h = 210
    ...
    pygame.draw.line(surface, SPLIT_LINE_BLUE,
                     (box.x + 16, box.y + 112), (box.x + 40, box.y + 112), 4)
    draw_text(surface, left_items[2], small_font, TEXT_COLOR, left_x, box.y + 103)

    pygame.draw.line(surface, SPLIT_LINE_YELLOW,
                     (box.x + 16, box.y + 142), (box.x + 40, box.y + 142), 4)
    ...
    pygame.draw.line(surface, PANEL_BORDER,
                     (divider_x, box.y + 10), (divider_x, box.bottom - 10), 1)


def draw_shortcuts_bar(surface, small_font, presentation_mode):
    text = ("Space Pause/Play   Right Step   F Fullscreen   "
...
def draw_presentation_banner(surface, fonts, paused,
                             blue_speed_factor, yellow_speed_factor, auto_demo_mode):
    ...
    draw_text(surface, "Presentation Mode", subtitle_font, TITLE_COLOR,
              box.x + 16, box.y + 12)
    draw_text(surface, f"Status: {'Paused' if paused else 'Running'}",
              small_font, SUBTLE_TEXT, box.x + 18, box.y + 50)
    draw_text(surface,
              f"Blue {blue_speed_factor:.2f}x   Yellow {yellow_speed_factor:.2f}x",
              small_font, SUBTLE_TEXT, box.x + 180, box.y + 50)
    draw_text(surface, f"Auto Demo {'On' if auto_demo_mode else 'Off'}",
              small_font, SUBTLE_TEXT, box.x + 18, box.y + 72)


def draw_scene(surface, vehicles, takt_display, buttons,
               blue_speed_factor, yellow_speed_factor, paused,
               show_labels, fullscreen, presentation_mode, auto_demo_mode):
    ...
    draw_text(surface,
              ...
              title_font, TITLE_COLOR, TITLE_X, TITLE_Y)
    draw_text(surface, f"Takt: {takt_display}",
              subtitle_font, TEXT_COLOR, TAKT_X, TAKT_Y)
    draw_text(surface, f"Version: {version.get_version()}",
              subtitle_font, TEXT_COLOR, VERSION_X, VERSION_Y)
    draw_text(surface, f"Status: {'Paused' if paused else 'Running'}",
              subtitle_font, TEXT_COLOR, STATUS_X, STATUS_Y)

    if presentation_mode:
        draw_presentation_banner(surface, fonts, paused,
                                 blue_speed_factor, yellow_speed_factor,
                                 auto_demo_mode)
    else:
        draw_header_controls(surface, fonts, buttons,
                             blue_speed_factor, yellow_speed_factor,
                             paused, show_labels, fullscreen,
                             presentation_mode, auto_demo_mode)
        draw_legend(surface, fonts, paused,
                    blue_speed_factor, yellow_speed_factor,
                    fullscreen, presentation_mode, auto_demo_mode)

    draw_input_main_line(surface, label_font,
                         show_labels=effective_show_labels)
    draw_branch_line(surface, label_font, "blue_branch",   "BLUE BRANCH",   "B",
                     show_labels=effective_show_labels)
    draw_branch_line(surface, label_font, "yellow_branch", "YELLOW BRANCH", "Y",
                     show_labels=effective_show_labels)
    ...
    for vehicle in vehicles:
        if vehicle.route == "main" and vehicle.main_pitch_float is not None:
            ...
            x, y = get_vehicle_xy("splitting", 0.0,
                                  vehicle.split_target_route, vehicle.split_progress)
            draw_vehicle(surface, x, y, 1.0, vehicle.color)
        elif vehicle.route == "merging":
            x, y = get_vehicle_xy("merging", 0.0,
                                  vehicle.merge_source_route, vehicle.merge_progress)
            draw_vehicle(surface, x, y, 1.0, vehicle.color)
...
def update_main_line(main_vehicles, delta_pitch_main):
    update_line_positions(main_vehicles, "main_pitch_float",
                          delta_pitch_main, MAIN_MIN_SPACING)
...
def update_branch_line(branch_vehicles, speed_factor, min_spacing, delta_pitch_main):
    ...
    update_line_positions(branch_vehicles, "branch_pitch_float",
                          branch_speed, min_spacing)


def update_output_main_line(out_vehicles, delta_pitch_main):
    update_line_positions(out_vehicles, "out_pitch_float",
                          delta_pitch_main, OUT_MAIN_MIN_SPACING)
...
def try_split_branch(vehicles, color_check, target_route, min_spacing):
    branch_vehicles   = [v for v in vehicles
                         if v.route == target_route
                         and v.branch_pitch_float is not None]
    splitting_vehicles = [v for v in vehicles
                          if v.route == "splitting"
                          and v.split_target_route == target_route]
...
def find_merge_candidate(vehicles, source_route):
    last_branch_pitch = (NUM_PITCHES_BLUE
                         if source_route == "blue_branch"
                         else NUM_PITCHES_YELLOW)
    candidates = [
        v for v in vehicles
...
def can_release_to_merge(vehicles):
    if any(v.route == "merging" for v in vehicles):
        return False
    out_vehicles      = [v for v in vehicles
                         if v.route == "out_main"
                         and v.out_pitch_float is not None]
    last_out_position = min(
        [v.out_pitch_float for v in out_vehicles], default=None)
...
def simulate_increment(vehicles, next_vehicle_id, elapsed_since_takt,
                       current_takt, merge_state,
                       blue_branch_speed_factor, yellow_branch_speed_factor, dt):
    elapsed_since_takt += dt
    delta_pitch_main    = dt / TAKT_TIME_MAIN

    main_vehicles = [v for v in vehicles
                     if v.route == "main" and v.main_pitch_float is not None]
    update_main_line(main_vehicles, delta_pitch_main)

    try_split_branch(vehicles, color_check=lambda v: v.is_blue(),
                     target_route="blue_branch",   min_spacing=BLUE_BRANCH_MIN_SPACING)
    try_split_branch(vehicles, color_check=lambda v: v.is_yellow(),
                     target_route="yellow_branch", min_spacing=YELLOW_BRANCH_MIN_SPACING)

    update_splitting_vehicles(vehicles, dt)

    blue_branch_vehicles   = [v for v in vehicles
                               if v.route == "blue_branch"
                               and v.branch_pitch_float is not None]
    yellow_branch_vehicles = [v for v in vehicles
                               if v.route == "yellow_branch"
                               and v.branch_pitch_float is not None]

    update_branch_line(blue_branch_vehicles,
                       speed_factor=blue_branch_speed_factor,
                       min_spacing=BLUE_BRANCH_MIN_SPACING,
                       delta_pitch_main=delta_pitch_main)
    update_branch_line(yellow_branch_vehicles,
    ...
    out_main_vehicles = [v for v in vehicles
                         if v.route == "out_main"
                         and v.out_pitch_float is not None]
...
def step_one_takt(vehicles, next_vehicle_id, elapsed_since_takt,
                  current_takt, merge_state,
    ...
    for _ in range(substeps):
        vehicles, next_vehicle_id, elapsed_since_takt, current_takt, merge_state = simulate_increment(
            vehicles, next_vehicle_id, elapsed_since_takt,
            current_takt, merge_state,
            blue_branch_speed_factor, yellow_branch_speed_factor, dt)
    return vehicles, next_vehicle_id, elapsed_since_takt, current_takt, merge_state
...
def toggle_auto_demo(auto_demo_mode, blue_branch_speed_factor, yellow_branch_speed_factor):
    if auto_demo_mode:
        ...
        auto_demo_timer    = 0.0
        return (True, auto_demo_index, auto_demo_timer,
                blue_branch_speed_factor, yellow_branch_speed_factor)
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    ...
                    if auto_demo_mode:
                        (auto_demo_mode, auto_demo_index, auto_demo_timer,
                         blue_branch_speed_factor,
                         yellow_branch_speed_factor) = toggle_auto_demo(
                             auto_demo_mode,
                             blue_branch_speed_factor,
        ...
        draw_scene(screen, vehicles, current_takt, buttons,
                   blue_speed_factor=blue_branch_speed_factor,
                   yellow_speed_factor=yellow_branch_speed_factor,
                   paused=paused, show_labels=show_labels,
                   fullscreen=fullscreen, presentation_mode=presentation_mode,
