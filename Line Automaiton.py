...
def compute_layout():
    ...
    step_guess = int(
        (total_width - BRANCH_START_OFFSET_X) / (NUM_PITCHES_MAIN + NUM_PITCHES_BLUE)
    )
...
def draw_vehicle(surface, x, y, scale, body_color):
    ...
    cabin_rect = pygame.Rect(
        int(x + body_w * 0.28), int(y - 13 * scale), cabin_w, cabin_h
    )
    ...
    pygame.draw.circle(
        surface,
        WHEEL_COLOR,
        (int(x + body_w * 0.20), int(y + 22 * scale)),
        wheel_d // 2,
    )
    pygame.draw.circle(
        surface,
        WHEEL_COLOR,
        (int(x + body_w * 0.78), int(y + 22 * scale)),
        wheel_d // 2,
    )
...
def draw_main_line(surface, label_font):
    ...
    label_rect = pygame.Rect(
        first_rect.x, first_rect.y - 70, last_rect.right - first_rect.x, 30
    )
    ...
    rail_rect = pygame.Rect(
        first_rect.x, int(rail_y), last_rect.right - first_rect.x, 14
    )
