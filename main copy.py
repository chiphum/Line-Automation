...
def draw_button(
    ...
    pygame.draw.rect(
        surface,
        ACCENT_BLUE if base_color == BUTTON_BLUE else PANEL_BORDER,
        rect,
        width=2,
        border_radius=8,
    )
...
def draw_header_controls(
    surface, fonts, buttons, blue_speed_factor, yellow_speed_factor, paused, show_labels
):
...
def draw_scene(
    ...
    draw_branch_line(
        surface, label_font, "blue_branch", "BLUE BRANCH", "B", show_labels=show_labels
    )
    draw_branch_line(
        surface,
        label_font,
...
def main():
    ...
    buttons = {
        ...
        "toggle_labels": pygame.Rect(
            CONTROL_PANEL_X + 1050, CONTROL_PANEL_Y + 68, 122, 28
        ),
    }
